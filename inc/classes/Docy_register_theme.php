...
class Docy_register_theme {
	...
	function admin_error() {
		$out = '<div class="notice notice-error is-dismissible docy-purchase-notice"><p>' . sprintf( wp_kses_post( __( 'The %s theme needs to be registered. %sRegister Now%s', 'docy' ) ), 'Docy', '<a href="' . admin_url( 'admin.php?page=docy') . '">' , '</a>' ) . '</p></div>';
	...
	function admin_notice() {
		$out = '<div class="notice is-dismissible docy-purchase-notice"><p>' .
                         sprintf( wp_kses_post( __( 'Purchase code is invalid. Need a license? %sPurchase Now%s', 'docy' ) ), '<a target="_blank" href="https://themeforest.net/item/docy-documentation-and-forum-wordpress-theme/31370838">', '</a>' ) .
                '</p></div>';
		if ( get_option('notice_dismissed') ) {
		    return;
        }
	...
	function form() {
		...
		<form action="" method="post" id="verify-envato-purchase" class="st-theme-register-form">
			...
			<input id="docy_purchase_code" name="docy_purchase_code" type="<?php echo esc_attr($type) ?>" value="<?php echo esc_attr( $license ); ?>" placeholder="<?php esc_attr_e( 'Enter your purchase code', 'docy' ); ?>" required>

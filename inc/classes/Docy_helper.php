...
class Docy_Helper_Class {
    ...
    public function logo() {
        ...
        $select_main_logo = function_exists('get_field') ? get_field('docy_header_type') : '';
        ...
        $is_main_logo_white = '';
        $is_main_logo_black = '';
        if ( ( docy_is_aesthetic_default() && !is_page() ) || ( is_singular('post') && $header_type == 'default') ) {
            $select_main_logo = 'white';
        }

        if ( docy_is_titlebar_default() ) {
            $select_main_logo = 'black';
        }

        if ( $select_main_logo == 'white' ) {
            $is_main_logo_white = 'main_logo';
        } elseif ( $select_main_logo == 'black' || docy_navbar_position() == 'static' ) {
    ...
    */
     function meta_css_render( $handle, $css_items ) {
         ...
         if ( !empty($opt['accent_solid_color_opt']) ) {
             $brand_color = $this->hex2rgba($opt['accent_solid_color_opt']);
             $dynamic_css .= "span#subscription-toggle a, .action-button-container.action-btns .bbp-admin-links li a.bbp-topic-reply-link{background: rgba($brand_color, 0.9); }";
             $dynamic_css .= ".direction_step{background: rgba($brand_color, 0.6); }";
             $dynamic_css .= ".direction_step + .direction_step:before{color: rgba($brand_color, 0.6); }";
             $dynamic_css .= ".header_search_keyword ul li a.has-bg:hover { background: rgba($brand_color, 0.8) }";

             // button hover shadow
             $btn_shadow_class = '.bbp-submit-wrapper #user-submit:hover, #new-post #bbp_reply_submit:hover, #new-post button#bbp_topic_submit:hover, .blog_comment_box .get_quote_form .thm_btn:hover';
             $dynamic_css .= "$btn_shadow_class {box-shadow: 0 20px 30px 0 rgba($brand_color, 0.24); }";
             $dynamic_css .= ".single_post_tags.post-tags a:hover {box-shadow: 0 10px 20px 0 rgba($brand_color, 0.24); }";
             $dynamic_css .= ".search-banner-light .header_search_keyword ul li a { border-color: rgba($brand_color, 0.2); }";
         }
